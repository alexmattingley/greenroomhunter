...
## 🚀 Next Up
**Improve the Tide block.** The tide chart is basically unusable on mobile — this is the most painful issue and the next thing we're working on.
- Break each day into its own chart with carousel functionality
- Better tooltips and drag functionality, plus time on the y-axis
...
| Improve tide chart — better tooltips, drag functionality, time on y-axis | 🔴 High | M |
...
| Item | Priority | Complexity |
| --- | --- | --- |
| Individual accounts (personal surf log) | 🟡 Medium | L |
| Add live weather readings (NOAA — [NCDC CDO Web Services](https://www.ncdc.noaa.gov/cdo-web/webservices/v2)) | 🟡 Medium | M |
| Create forecast using ECMWF model ([Open-Meteo Marine API](https://open-meteo.com/en/docs/marine-weather-api)) | 🔴 High | L |
